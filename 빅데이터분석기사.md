...
- predict_proba : 평가 지표가 roc_auc_score인 경우, 확률값 예측인 경우
- [:, 1] 사용해야 함 
...
x_train = combined[combined['train'] == 1].drop('train', axis=1)
x_test = combined[combined['train'] == 0].drop('train', axis=1)

# 수치형 변수는 Regressor, 범주형 변수는 Classifier
...
pd.DataFrame({'ID':X_test['id'], 'charges':pred}).to_csv('수험번호.csv', index=False)